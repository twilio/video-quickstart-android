...
class CameraCapturerCompat(
    ...
    fun switchCamera() {
        ...
        } else Log.w("CameraCapturerCompat", "Front and back cameras need to both be available in order to switch between them")
    }

    fun isFrontCamera(): Boolean {
        if (frontCameraId != null && backCameraId != null) {
            if(cameraCapturer != null) return cameraCapturer?.cameraId == frontCameraId
            if(camera2Capturer != null) return cameraCapturer?.cameraId == frontCameraId
            return false
        } else {
            Log.w("CameraCapturerCompat", "Front and back cameras need to both be available in order to switch between them")
            return false
        }
    }

    fun updateCameraParametersss(flashToggler: CameraParameterUpdater) {
        cameraCapturer?.updateCameraParameters(flashToggler)
        camera2Capturer?.updateCaptureRequest { captureRequestBuilder: CaptureRequest.Builder ->
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                cameraManager?.let { cameraManager ->
                    val cameraCharacteristics: CameraCharacteristics = cameraManager.getCameraCharacteristics(camera2Capturer!!.cameraId)
                    val flashEnabled = cameraCharacteristics.get(CameraCharacteristics.FLASH_INFO_AVAILABLE)
                    if (flashEnabled != null && flashEnabled) {
                        captureRequestBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_TORCH)
                    }
                }
//                camera2Capturer.updateCaptureRequest(
//                        CaptureRequestUpdater { captureRequestBuilder -> captureRequestBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_TORCH) })
            }
        }
    }
        ...
        private fun isCameraIdSupported(context: Context, cameraId: String): Boolean {
            return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                cameraCharacteristics = try {
                    ...
                    Log.e("CameraCapturerCompat", e.message, e)
                    return false
                } ?: return false
                /*
